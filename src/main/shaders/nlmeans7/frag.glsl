...
highp float placeAndCompare(ivec2 center, vec3 [9]compare, inout vec3 insample){
  ...
  highp float diff;
  diff = dot(abs(rgbsample(center+ivec2(-1,1)) -compare[0]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(0,1)) -compare[1]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(1,1)) -compare[2]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(-1,0)) -compare[3]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(0,0)) -compare[4]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(1,0)) -compare[5]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(-1,-1)) -compare[6]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(0,-1)) -compare[7]), dotsums)/3.0;
  acc += diff;
  diff = dot(abs(rgbsample(center+ivec2(1,-1)) -compare[8]), dotsums)/3.0;
  acc += diff;
