...
genPlaceCompare += '{\n'
genPlaceCompare += `  highp float acc = 0.0;\n`
...
sv.forEach((sample, i) => {
  main += ` finalKernalWeights[${i}] /= weightsum;\n`
  main += ` acc += finalKernalWeights[${i}]*finalKernalSamples[${i}];\n`
})
main += `
  if (vnoise) {
...
console.log(`SAMPLE KERNAL N = ${kn}`)
console.log(`SAMPLE KERNAL SIZE = ${ks}`)
console.log(`SAMPLE KERNAL AREA = ${ksq}`)

console.log(`SEARCH AREA N = ${sn}`)
console.log(`SEARCH AREA SIZE = ${ss}`)
console.log(`SEARCH AREA AREA = ${ssq}`)

fs.writeFileSync('frag.glsl', final)
