...
module.exports=function(kn,sn){
  ...
  genPlaceCompare += `  highp float diff;\n`
  kv.forEach((sample, i) => {
    genPlaceCompare += `  diff = dot(abs(rgbsample(center+ivec2(${sample.x},${sample.y})) -compare[${i}]), dotsums)/${ks}.0;\n`
    genPlaceCompare += `  acc += diff;\n`
  })
  genPlaceCompare += `  insample = rgbsample(center);\n`
  genPlaceCompare += `  acc /= ${ks}.0;\n`
  genPlaceCompare += `  return pow(acc, power);\n`
  genPlaceCompare += '}\n'
